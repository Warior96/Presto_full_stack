                ...
                <div class="col-12 container-img-card p-0">
                    ...
                    <div class="black-opacity radius d-flex flex-column justify-content-center align-items-center">
                        @switch($categoria->name)
                            @case('Elettronica')
                                <h4 class="mb-4 c-2">La tua lavatrice ha iniziato a fare i capricci? Sostituiscila con una della categoria "{{$categoria->name}}"</h4>
                                @break
                                @case('Abbigliamento')
                                    <h4 class="mb-4 c-2">Hai l'armadio vuoto? Riempilo con gli articoli della categoria "{{$categoria->name}}"</h4>
                                    @break
                                @case('Bellezza')
                                    <h4 class="mb-4 c-2">Cura la tua pelle con i prodotti della categoria "{{$categoria->name}}"</h4>
                                    @break
                                @case('Giardinaggio')
                                    <h4 class="mb-4 c-2">Hai il pollice verde? Dai sfogo alla tua fantasia con i prodotti della categoria "{{$categoria->name}}"</h4>
                                    @break
                                @case('Giocattoli')
                                    <h4 class="mb-4 c-2">I giochi sparsi per casa non sono abbastanza? Aggiungine degli altri con gli articoli della categoria "{{$categoria->name}}"</h4>
                                    @break
                                @case('Sport')
                                    <h4 class="mb-4 c-2">Sei un tipo atletico? Dai un'occhiata alla categoria "{{$categoria->name}}"</h4>
                                    @break
                                @case('Tecnologia')
                                    <h4 class="mb-4 c-2">Dispositivo vecchio? Guarda le nostre offerte nella categoria "{{$categoria->name}}"</h4>
                                    @break
                                @case('Libri e riviste')
                                    <h4 class="mb-4 c-2">Trova il libro dei tuoi sogni! Dai un occhiata alla categoria "{{$categoria->name}}"</h4>
                                    @break
                                @case('Accessori')
                                    <h4 class="mb-4 c-2">I piccoli dettagli a volte fanno la differenza, cerca il tuo stile all'interno della categoria "{{$categoria->name}}"</h4>
                                    @break
                                @case('Motori')
                                    <h4 class="mb-4 c-2">Stai vendendo il tuo catorcio o ne stai cercando un altro? Scoprilo nella categoria "{{$categoria->name}}"</h4>
                                    @break
                        @endswitch

                        {{-- search --}}
                        <form action="{{ route('article.search') }}" method="GET" role="search" class="d-flex w-75">
        ...
        <div class="row justify-content-center p-0 position-relative">
            <div class="col-12 container-little-img-card p-0">
                ...
                <div class="little-black-opacity p-0 radius">
                    <h5 class="c-2 position-absolute top-50 start-50 translate-middle text-center">{{$categoria->name}}</h5>
                </div>
            </div>
        </div>
    </swiper-slide>
